<template>
	<view class="holdon" >
		<image  class="ball" :style="'left:'+(moveX == 0 & x>0? x:moveX)+'px;top:'+(moveY == 0 & y>0? y:moveY)+'px'"	 
			@click="handleToChat"	@touchstart="drag_start" @touchmove.prevent="drag_hmove" :src="image"  mode="aspectFill">
		</image>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			drag_hmove(event){
				...
				// this.$emit('moveBall',{x:this.moveX,y:this.moveY})
			},
			handleToChat() {
				uni.navigateTo({
					url: '/pages/chat/chat?auto=1',
